...
type Props = {
  onClose: () => void;
  onSave: (data: { title: string; targetAmount: number; deadline?: string; color: string }) => void;
};
...
export default function GoalModal({ onClose, onSave }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-slate-900/60 dark:bg-[#09090B]/90 backdrop-blur-md">
      <div 
        className="w-full max-w-md bg-white dark:bg-[#1C1C1F] border border-transparent dark:border-white/5 rounded-t-[40px] sm:rounded-3xl p-6 pb-20 sm:pb-6 shadow-2xl animate-in slide-in-from-bottom-2 fade-in"
      >
